package com.chivorn.hum.sample.data;
...
public class DataHelper {
    ...
    private static List<ColorSuggestion> sColorSuggestions = new ArrayList<>(Arrays.asList(
    ...
    public static void findSuggestions(Context context, String query, final int limit, final long simulatedDelay, final OnFindSuggestionsListener listener) {
        new Filter() {
    ...
    public static void findSuggestions(Context context, final List<String> listData, String query, final int limit, final long simulatedDelay, final OnFindSuggestionsListener listener) {
        new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence constraint) {
                ...
                if (!(constraint == null || constraint.length() == 0)) {
                    ...
                    List<ColorSuggestion> sColorSuggestions = new ArrayList();
                    for (int i = 0; i < listData.size(); i++) {
                        ColorSuggestion colorSuggestion = new ColorSuggestion(listData.get(i));
                        sColorSuggestions.add(colorSuggestion);
                    }

                    for (ColorSuggestion suggestion : sColorSuggestions) {
                        if (suggestion.getBody().toUpperCase().contains(constraint.toString().toUpperCase())) {
